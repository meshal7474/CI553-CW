...
public class Basket extends ArrayList<Product> implements Serializable {
    ...
    private boolean saleApplied = false; // Flag to indicate if sale has been applied
    private double tipAmount = 0.0; // Tip amount
    ...
    /**
     * Set the sale status
     * @param applied true if sale has been applied
     */
    public void setSaleApplied(boolean applied) {
        this.saleApplied = applied;
    }
    ...
    public String getDetails() {
        ...
        if (this.size() > 0) {
            ...
            fr.format("%s%7.2f\n", csign, total);
            if (saleApplied) {
                fr.format("The sale has been placed\n");
            }
            if (tipAmount > 0) {
                fr.format("Tip                         ");
                fr.format("%s%7.2f\n", csign, tipAmount);
